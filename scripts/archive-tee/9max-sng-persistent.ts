/**
 * 9-Max Gold SNG Persistent E2E Test — Resumable, Reusable, Seat-Shuffling
 *
 * Uses persistent keypairs (tests/keys/player1-9.json).
 * Saves state to scripts/9max-sng-state.json.
 * On --reuse, SHUFFLES seat assignments so players sit in different seats
 * than game 1. This verifies permissions are properly reset for new occupants.
 *
 * Usage:
 *   npx ts-node scripts/9max-sng-persistent.ts              # Run from saved state
 *   npx ts-node scripts/9max-sng-persistent.ts --fresh       # Force new table
 *   npx ts-node scripts/9max-sng-persistent.ts --from=4      # Resume from step
 *   npx ts-node scripts/9max-sng-persistent.ts --reuse       # Reuse with shuffled seats
 */
import {
...
const STATE_FILE = 'j:/Poker/scripts/9max-sng-state.json';

const MAX_PLAYERS = 9;
const SNG_TIER_GOLD = 3;
const GAME_TYPE_9MAX = 2; // SitAndGo9Max
const STARTING_STACK = 1500;
const GOLD_ENTRY = 75_000_000;   // 0.075 SOL
const GOLD_FEE = 25_000_000;     // 0.025 SOL
const GOLD_TOTAL = GOLD_ENTRY + GOLD_FEE; // 0.1 SOL

// Seat assignment order: game 1 = [0,1,2,...8], game 2+ = shuffled
const SEAT_ORDER_GAME1 = [0, 1, 2, 3, 4, 5, 6, 7, 8];
const SEAT_ORDER_GAME2 = [5, 2, 8, 0, 6, 1, 7, 3, 4]; // deterministic shuffle
...
async function main() {
  ...
  // Load keypairs first, assign seats based on game number later
  const keypairs = Array.from({ length: MAX_PLAYERS }, (_, i) =>
    loadKey(`j:/Poker/tests/keys/player${i + 1}.json`),
  );
  // Seat assignment: game 1 = sequential, game 2+ = shuffled
  // This tests that permissions, markers, etc. work for different occupants
  const existingState = loadState();
  const gameNum = REUSE ? ((existingState?.gameNumber || 1) + 1) : (existingState?.gameNumber || 1);
  const seatOrder = gameNum <= 1 ? SEAT_ORDER_GAME1 : SEAT_ORDER_GAME2;
  const players = keypairs.map((kp, i) => ({
    kp,
    seatIndex: seatOrder[i],
    name: `P${i}`,
  }));

  log('╔══════════════════════════════════════════════════════════════╗');
  log('║   9-MAX GOLD SNG PERSISTENT E2E TEST                        ║');
  log('╚══════════════════════════════════════════════════════════════╝');
  log(`Deployer: ${deployer.publicKey.toBase58()} (${((await l1.getBalance(deployer.publicKey)) / 1e9).toFixed(4)} SOL)`);
  for (const p of players) {
    log(`${p.name}: ${p.kp.publicKey.toBase58().slice(0, 16)}... → seat ${p.seatIndex} (${((await l1.getBalance(p.kp.publicKey)) / 1e9).toFixed(4)} SOL)`);
  }
  log(`Buy-in: ${GOLD_TOTAL / 1e9} SOL (entry=${GOLD_ENTRY / 1e9}, fee=${GOLD_FEE / 1e9}) × 9 = ${(GOLD_TOTAL * 9) / 1e9} SOL total`);
  log(`Seat order: [${seatOrder.join(',')}] (game #${gameNum})`);
  ...
  // ═══ STEP 1: CREATE TABLE ═══
  if (startStep <= 1) {
    log('\n═══ STEP 1: Create 9-Max Gold SNG Table ═══');
    ...
    if (existing) { pass('Table already exists'); }
      ...
      data.writeUInt8(GAME_TYPE_9MAX, 40); // SitAndGo6Max
      data.writeUInt8(0, 41);               // Micro tier placeholder
      data.writeUInt8(MAX_PLAYERS, 42);
      data.writeUInt8(SNG_TIER_GOLD, 43);
      await sendTx(l1, [
  ...
  if (startStep <= 2) {
    log('\n═══ STEP 2: Init 9 Seats + Create Permissions ═══');
  ...
  if (startStep <= 3) {
    log('\n═══ STEP 3: Register + Join 9 Players ═══');

    // On reuse: wait for table to be program-owned + Waiting phase on L1
    // (crank must finish undelegate + distribute + reset first)
    if (REUSE) {
      log('  Waiting for crank to finish processing previous game...');
      for (let poll = 0; poll < 120; poll++) {
        const ti = await l1.getAccountInfo(tablePda);
        if (ti && ti.owner.equals(PROGRAM_ID)) {
          const phase = Buffer.from(ti.data)[OFF.PHASE];
          if (phase === Phase.Waiting) {
            pass('Table back on L1 in Waiting phase');
            break;
          }
          log(`  L1 phase=${PHASE_NAMES[phase] || phase}... ${poll + 1}/120`);
        } else {
          if (poll % 10 === 0) log(`  Table still delegation-owned... ${poll + 1}/120`);
        }
        await sleep(3000);
        if (poll === 119) { fail('Table reuse', 'Timed out waiting for crank to reset table'); process.exit(1); }
      }
    }
...

    {
      ...
      if (ti) {
        ...
        if (cp === MAX_PLAYERS) pass('All 9 players joined');
  ...
  if (startStep <= 4) {
    ...
    if (tableInfo && tableInfo.owner.equals(DELEGATION_PROGRAM_ID)) {
      ...
      // CrankTallyER — delegate to TEE so start_game can record crank actions
      {
        ...
        if (tallyInfo && tallyInfo.owner.equals(PROGRAM_ID)) {
          ...
          log('  CrankTallyER not found (will be created by crank)');
        }
      }
  ...
  if (startStep <= 6) {
    ...
    const expectedFees = GOLD_FEE * MAX_PLAYERS;
    ...
    log(`  Expected prize pool: ${GOLD_ENTRY * MAX_PLAYERS} (${(GOLD_ENTRY * MAX_PLAYERS) / 1e9} SOL)`);
  ...
  // ═══ RESULTS ═══
  log('\n╔══════════════════════════════════════════════════════════════╗');
  log('║   9-MAX GOLD SNG TEST RESULTS                               ║');
  log('╚══════════════════════════════════════════════════════════════╝');
  ...
  if (failCount === 0) {
    log('\n  🎉 ALL TESTS PASSED!');
    log('  To reuse: npx ts-node scripts/9max-sng-persistent.ts --reuse');
  } else {
    log(`\n  ⚠️  ${failCount} test(s) failed`);
    log('  To resume: npx ts-node scripts/9max-sng-persistent.ts');
  }
}

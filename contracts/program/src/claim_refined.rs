...
/// Claim all rewards (unrefined @ 90% after tax + 100% refined)
/// Tax redistributed to other unclaimed holders (ORE pattern)
/// Cannot claim unrefined without also claiming refined — enforced here
pub fn process_claim_refined(accounts: &[AccountInfo<'_>], _data: &[u8]) -> ProgramResult {
    // Parse accounts
    ...
    // Calculate pending refined rewards before claiming
    ...
    // Total = net unrefined + all refined (refined has no additional tax)
    ...
    // Clear all amounts and reset debt
